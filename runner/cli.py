...
def launch_mission_once(
    ...
    existing_pid = active_pid_from_state(
        mission_name
    )
    ...
    print(
        f"Started {source_label} {mission_name} "
    ...
    while True:
        ...
        while process.poll() is None:
            if stop_requested:
                ...
                print(
                    f"Stopped mission {mission_name}"
                )
                return 0
        ...
        previous_reason = current_state.get(
            "last_exit_reason"
        )
...
def status_mission(mission_name: str) -> int:
    ...
    if state.get("stopped_at"):
        print(
            f"Stopped: {state.get('stopped_at')}"
        )
...
def logs_mission(
    ...
    if not path.exists():
        print(
            f"No logs found for {mission_name}"
        )
        return 1
...
def stop_mission(
    ...
    if not is_process_group_leader(
        mission_pid
    ):
        print(
    ...
    if not stopped:
        ...
        try:
            ...
            update_state(
                ...
                stopped_at=utc_now(),
                last_exit_reason=(
                    "kill_permission_error"
                ),
                reason=str(exc),
            )

            print(
                f"Could not force-stop mission: "
                f"{exc}",
                file=sys.stderr,
            )
            return 1
...
def add_candidate_flag(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Load the mission from "
            "agents/candidates/ instead of missions/"
        ),
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    add_candidate_flag(start_parser)
    ...
    add_candidate_flag(supervise_parser)
